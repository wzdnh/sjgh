...
<div class=view_larger_image_bac onclick="view_larger_image_hidden()">
    <img src="${sessionScope.transactionOfThings.road}">
</div>
            ...
            <div class="task_all">
                <div class="money_event">
                    ...
                    <div class="event_home">
                        ...
                        <span class="event">${sessionScope.transactionOfThings.event_tags}</span>
                    </div>
                </div>
                ...
                <div class="description_img">
                    <img id="img_id" src="${sessionScope.transactionOfThings.road}" onclick="view_larger_image()">
                </div>
                <div class="exhibition_details_footer">
                    <div class="user_imformation">
                        <img src="${sessionScope.transactionOfThings.user_er.head_portrait}" width="45px" height="45px">
                        <div class="Name_LegalTimes">
                            ...
                            <div class="legaltimes">
                                ...
                                <span class="times">${sessionScope.transactionOfThings.user_er.performance}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="comment_input">
                        ...
                        <div class="comment_input_right">
                            <form class="comment_input_despriction" method="post">
                                <textarea class="swjy_comment"   placeholder="我也说两句"></textarea>
                                <button  type="button"  onclick="swjy_content_update()">发送</button>
                            </form>
                        </div>
                    </div>
                    <!-- 输入评论end -->
...
<%--<form id="comment_swjy" method="post">
    <input type="text" name="content">
    <input type="button" onclick="swjy_content_update()" value="点我评论">
</form>--%>
</body>
...
<script>
    ...
    //事物交易评论提交
    function swjy_content_update() {
        var formData = new FormData($(".comment_input_despriction")[0]);
        formData.append("user_id", '${sessionScope.user.user_id}');
            /*//放入此事件代办的id*/
        formData.append("swjy_id", ${sessionScope.transactionOfThings.id});
        formData.append("content", $(".swjy_comment").val());
        //放入这个产品的发布的人的id
        formData.append("commented_user_id", ${sessionScope.transactionOfThings.user_id});
        $.ajax({
            type: "POST",
            url: "<%= basePath%>/re_detail/swjy_comment_add",
            data: formData,
    ...
    function swjy_content_list(){
        var formDataTwo = new FormData();
        //放入此事件代办的id
        formDataTwo.append("swjy_id", ${sessionScope.transactionOfThings.id})
        $.ajax({
            type: "POST",
            url: "<%= basePath%>/re_detail/swjy_comment_list",
            data: formDataTwo,
            //async: false,//要求同步 不是不需看你的需求,
            /*false;避开jQuery对formdata的默认处理*/
            processData: false,
                    ...
                    $('.comment').append(
                        "<div class='comment_onefFloor'>" +
                            ...
                            "<div class='comment_right'>" +
                                ...
                                "</p>" +
                                "<p formatter='toDate' class='comment_time_day'>" + time +
                                "</p>" +
    ...
    function receive() {
        ...
        formDataTwo.append("type_id", "${sessionScope.transactionOfThings.id}");
        formDataTwo.append("invoice_user_id", "${sessionScope.transactionOfThings.user_id}");
        formDataTwo.append("type", "事物交易");
        formDataTwo.append("state", "${sessionScope.transactionOfThings.state}");
        $.ajax({
            type: "POST",
            url: "<%= basePath%>/orders/other_receive",
            data: formDataTwo,
            //async: false,//要求同步 不是不需看你的需求,
            /*false;避开jQuery对formdata的默认处理*/
            processData: false,
