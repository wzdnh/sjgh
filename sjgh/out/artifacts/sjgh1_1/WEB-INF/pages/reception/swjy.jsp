...
<head>
    ...
    <link rel="stylesheet" href="<%=request.getContextPath()%>/static/reception/css/swjy.css">
    ...
    <div class="loginSign_search">
        <div class="search">
            <div class="search_input">
                <form id="swjy_search" class="a" method="post">
                    <input id="search_input" type="text" name="swjy_keyWords">
                </form>
                <a onclick="swjy_search_update_page_info()">
    ...
    <nav>
        <div class="nav_center">
            ...
            <div class="big_keyword">
                ...
                <div class="swjy">
                    <a href="/re_seconds/swjy_page"><span class="big_keyword_two" style="color: #F18216">事物交易</span></a>
                </div>
            </div>
            <div class="main_img_home">
                <div class="main_img">
                    <img class="main_img" src="<%=request.getContextPath()%>${sessionScope.officialPicture.get(18).road}" />
                </div>
            </div>
            <div class="location_home">
                <div class="location_center">
                    <span class="location_center_title">热门：</span>
                    <a href="javascript:void(0);" onclick="lable_search_page_info('学习用品')"><span class="location_center_keywords">学习用品</span></a>
                    <a href="javascript:void(0);" onclick="lable_search_page_info('生活用品')"><span class="location_center_keywords">生活用品</span></a>
                    <a href="javascript:void(0);" onclick="lable_search_page_info('其他用品')"><span class="location_center_keywords">其他用品</span></a>
                </div>
            </div>
        </div>
    </nav>
...
<!--手机端自动跳转-->
<script src="<%=request.getContextPath()%>/static/reception/js/mb_seconds_swjy.js" type="text/javascript"></script>
...
<script type="text/javascript">
     ...
     *
     * */
    function swjy_search_update_page_info() {
        //放入输入的关键词
        var formData = new FormData($('#swjy_search')[0]);
        var swjy_keyWords = formData.get("swjy_keyWords");
        $.ajax({
            url: "<%=basePath%>/search/swjy_search_page_info",
            type: "post",
            data: formData,
            processData: false,
            /*必须false才会自动加上正确的content-type*/
            ...
            success: function (data) {
                ...
                $("#demo3").paginate({
                    ...
                    onChange: function a(currentPage) {//回调函数,参数为当前点击的页码
                        ...
                        swjy_search_update(swjy_keyWords, currentPage); //此函数，传入页码，获取相关信息
                    }
                });
                if (pageCount != 0) {
                    //,点击第一页
    ...
    //搜索部分的ajax
    function swjy_search_update(swjy_keyWords, currentPage) {
        //将页码发到后台,传回分页的列表,在渲染到对应位置
        //放入输入的关键词
        var formData = new FormData();
        formData.append("swjy_keyWords", swjy_keyWords);
        formData.append("currentPage", currentPage);
        $.ajax({
            type: "POST",
            url: "<%= basePath%>/search/swjy",
            data: formData,
            //async: false,//要求同步 不是不需看你的需求,
            /*false;避开jQuery对formdata的默认处理*/
            processData: false,
    ...
    $('#search_input').keyup(function () {
        ...
        if (searchKeyword != "") {
            ...
            $.ajax({
                url: "<%=basePath%>/search/swjy_search_review",
                type: "post",
                data: formData,
                dataType: "json",
                //async: false,//要求同步 不是不需看你的需求,
                ...
                success: function (data) {
                    ...
                    var arr = data.swjySearchPreview;
                    for (var i = 0; i < data.swjySearchPreview.length; i++) {
                        var describe = arr[i];
    ...
    $("#search_input").blur(function () {
        setTimeout(function(){$('.search_dynamic_preview li').remove()}, 300);
    });
    ...
    function lable_search_page_info(lable) {
        ...
        $.ajax({
            url: "<%=basePath%>/search/swjy_lable_search_page_info",
            type: "post",
            data: formData,
            processData: false,
            /*必须false才会自动加上正确的content-type*/
    ...
    function lable_search(lable, currentPage) {
        ...
        formData.append("currentPage", currentPage);
        $.ajax({
            url: "<%=basePath%>/search/swjy_lable_search",
            type: "post",
            data: formData,
            processData: false,
            contentType: false,
            success: function (data) {
                var arr = data.swjyLableSearchList;
                //清空，防止回显的数据叠加
                $('.task_all').remove();
                for (var i = 0; i < data.swjyLableSearchList.length; i++) {
                    var id = arr[i].id;
                    var name = arr[i].user_er.user_name;
                    var describe = arr[i].event_describe;
                    var tage = arr[i].event_tags;
